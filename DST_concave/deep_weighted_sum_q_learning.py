"""
Vector-valued Deep Q-Learning with weighted-sum action selection.
"""
...
def train_deep_weighted_sum_q(
    ...
    **training_kwargs,
    ...
    weights = np.array(
        [time_weight, treasure_weight], dtype=float
    )

    def score_function(normalized_q_vectors):
        return normalized_q_vectors @ weights
...
def _save_deep_weighted_sum_plots(
    results_by_weight,
    true_front,
    final_policy_front,
    archive_front,
    final_coverage,
    coverage,
    output_dir="results/deep_weighted_sum",
):
    ...
    ax.set_title("Deep Weighted-Sum Q-Learning")
    ax.grid(True, color="#cccccc", linewidth=0.7, alpha=0.8)
    ax.legend()
    fig.tight_layout()
    fig.savefig(
        os.path.join(output_dir, "deep_weighted_sum_pareto_front.png"),
        dpi=150,
        bbox_inches="tight",
        facecolor="white",
    )
    ...
    metric_specs = [
        ("Hypervolume", 2, 3, "HV vs Timestep (higher = better)", "deep_weighted_sum_hv.png"),
        ("IGD", 4, 5, "IGD vs Timestep (lower = better)", "deep_weighted_sum_igd.png"),
        (
            "Epsilon indicator",
            6,
            7,
            "Epsilon vs Timestep (lower = better)",
            "deep_weighted_sum_epsilon.png",
        ),
    ]
    ...
    print(f"\nSaved deep weighted-sum plots in: {output_dir}")
...
def run_all_deep_weighted_sum_weights(
    ...
    for index, weight in enumerate(weights):
        time_weight, treasure_weight = weight
        print(
            f"[{index + 1}/{len(weights)}] "
            f"weights=({time_weight:.2f}, {treasure_weight:.2f})"
        )
        result = train_deep_weighted_sum_q(
    ...
    print("\nCombined deep weighted-sum final-policy front:")
