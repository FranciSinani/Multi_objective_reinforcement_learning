"""
Vector-valued Deep Q-Learning with OWA action selection.
"""
...
DEEP_OWA_EXPERIMENT_SEED = 8


def train_deep_owa_q(
    owa_weights,
    **training_kwargs,
):
    ...
    def score_function(normalized_q_vectors):
        ordered = np.sort(normalized_q_vectors, axis=1)[:, ::-1]
...
def _save_deep_owa_plots(
    ...
    ax.set_title("Deep OWA Q-Learning")
    ax.grid(True, color="#cccccc", linewidth=0.7, alpha=0.8)
    ax.legend()
    fig.tight_layout()
    fig.savefig(
        os.path.join(output_dir, "deep_owa_pareto_front.png"),
        dpi=150,
        bbox_inches="tight",
        facecolor="white",
    )
    ...
    metric_specs = [
        ("Hypervolume", 2, 3, "HV vs Timestep (higher = better)", "deep_owa_hv.png"),
        ("IGD", 4, 5, "IGD vs Timestep (lower = better)", "deep_owa_igd.png"),
        (
            "Epsilon indicator",
            6,
            7,
            "Epsilon vs Timestep (lower = better)",
            "deep_owa_epsilon.png",
        ),
    ]
    ...
    print(f"\nSaved deep OWA plots in: {output_dir}")
...
def run_all_deep_owa_weights(
    ...
    for index, weight in enumerate(weights):
        print(
            ...
            f"OWA weights=({weight[0]:.2f}, {weight[1]:.2f})"
        )
        result = train_deep_owa_q(
            owa_weights=weight,
    ...
    print("\nCombined deep OWA final-policy front:")
